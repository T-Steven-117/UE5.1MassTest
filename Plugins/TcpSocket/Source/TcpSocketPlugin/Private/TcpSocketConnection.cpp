...
bool ATcpSocketConnection::SendData(int32 ConnectionId /*= 0*/, FString DataToSend)
{
	/*TCHAR* seriallizedChar = DataToSend.GetCharArray().GetData();
	int32 size = FCString::Strlen(seriallizedChar) + 1;
	int32 sent = 0;*/
	TCHAR* SendMessage = DataToSend.GetCharArray().GetData();
	...
	//TArray<uint8>msg= (uint8*)TCHAR_TO_UTF8(SendMessage);
	output.Add(0);
	if (TcpWorkers.Contains(ConnectionId))
	{
		if (TcpWorkers[ConnectionId]->isConnected())
		{
			TcpWorkers[ConnectionId]->AddToOutbox(output);
			return true;
		}
		else
		{
